...
class AppServiceProvider extends ServiceProvider
{
    ...
    {
        RateLimiter::for('upload', function (Request $request): array {
            ...
            $ipLimit = Limit::perMinute(120)->by('ip:'.$request->ip());
            ...
            // Unauthenticated requests (rate limiter runs before auth guard)
            // fall back to IP-based key.
            $userId = $request->user()?->id;
            $userKey = $userId !== null ? 'user:'.$userId : 'ip:'.$request->ip();
            $userLimit = Limit::perDay(10000)->by($userKey);
